...
void UpdateBalls(BallContainer *bc) {
    ...
    for (int i = 0; i < bc->ballsCapacity; i++) {
        ...
        //Wall collisions
        const float biSize = bi->size;
        bool applyRestitution = false;
        if (!bi->hitWall && (bi->position.x - biSize < 0.f || bi->position.x + biSize > (float) GetScreenWidth())) {
            newVel.x *= -1.f;
            bi->hitWall = true;
            applyRestitution = true;
        } else if (!bi->hitWall && (bi->position.y - biSize < 0.f || bi->position.y + biSize > (float) GetScreenHeight())) {
            newVel.y *= -1.f;
            bi->hitWall = true;
            applyRestitution = true;
        } else
